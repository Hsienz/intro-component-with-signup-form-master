...
const Input = (props: { placeholder: string, errorMsg: string }) => {
	return (
		<div className="relative flex-col">
			...
			/>
			<img src={iconError} alt="" className="absolute right-3 top-0 translate-y-1/2 hidden transition-all" />
			<p className="text-xs font-sans invisible text-right text-Red">{props.errorMsg}</p>
		</div>
	);
};
...
const Card = () => {
	return (
		<div className="flex flex-col text-center w-[320px] mx-8 my-16">
			...
			<div className="mt-12 bg-Blue w-full h-24 rounded-xl flex flex-col justify-center items-center">
				{" "}
				<p className="w-48 text-white font-light">
			...
			<div className="bg-white rounded-xl mt-8">
				...
				>
					<Input placeholder="First Name" errorMsg="First name cannot be empty" />
					<Input placeholder="Last Name" errorMsg="Last name cannot be empty"/>
					<Input placeholder="Email Address" errorMsg="Look like this is not an email"/>
					<Input placeholder="Password" errorMsg="Password cannot be empty"/>
					<button
						type="button"
